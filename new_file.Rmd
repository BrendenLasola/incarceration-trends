...
Things to do:
2.) Bar graph for the percentages based on each race and the total population for both jail and prison
...
Looking at the difference for each race over time, have they increased or decreased?
Doing this for each race and for jail/prison
```{r}
...
#  geom_line(aes(x = year, y = white_prison_pop), size = 1, color = 'green') +
#  xlab('Years') +
#  ylab('Total in Prison') +
# ggTitle('Change over time for black(c) and white people in Prison')
...
#Looking at the data, for only 2016. The reason for 2016 is because that is the latest date with the most amount
#of data
incar_2016 <- incar_trends %>%
  filter(year == '2016') 

#Table
urban_count <- table(incar_2016$urbanicity)
urban_table <- kable(urban_count,caption = 'Count of Urbanicity')
...
#Melted Urban Jail
urban_jail <- melt(urban_jail)

#Urban Jail Graphs
jail_graph <- ggplot(urban_jail,aes(x = urbanicity, y = value)) +
  geom_bar(aes(fill = variable), stat = 'identity', position = 'dodge') +
  scale_y_continuous('Jail Pop') + 
  xlab('Urbanicity') + 
  ylab('Percent') + 
  labs(fill='Race Pop',title = 'Total Pop in Jail for each Urbanicity')
...
#Melted Urban Prison
urban_prison <- melt(urban_prison)

#Graph of Urban Prison
prison_graph <- ggplot(urban_prison,aes(x = urbanicity, y = value)) +
  geom_bar(aes(fill = variable), stat = 'identity', position = 'dodge') +
  scale_y_continuous('Prison Pop') + 
  xlab('Urbanicity') + 
  ylab('Percent') + 
  labs(fill='Race Pop',title = 'Total Pop in Prison for each Urbanicity')


#Looking at the total amount of people for each urbanicity
...
total_graph <- ggplot(urban_year, aes(x = urbanicity,y = total_pop_15to64)) +
  geom_bar(stat = 'identity', fill = 'black') + 
  xlab('Urbanicity') + 
  ylab('Total Pop') +
  labs(title = 'Total Population for each Urbanicity')
```

So what we have hear is, how many people in each race is in jail for each urbanicity and same thing with prison.
The last thing I have is how many people are in each urbanicity) 
